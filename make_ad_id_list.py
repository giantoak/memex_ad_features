...
prices = pandas.read_csv('data/escort_cdr_2/rates-text.tsv', sep='\t', names=['ad_id','price'])
ad_ids = set(prices['ad_id'])
all_ads = set(pandas.read_csv('ad_id_list.csv', names=['ad_id'])['ad_id'])
out_with_price = []
out_no_price = []
fraction = .0001
...
df = pandas.DataFrame(out_with_price)
df['content'] = df['content'].apply(lambda x: x.strip('"'))
df.to_csv('data/temp/ad_ids_with_price.csv', index=False)

df_no_price=pandas.DataFrame([i for i in np.random.choice(out_no_price,1000000)])
#df_no_price = pandas.DataFrame(out_no_price)
